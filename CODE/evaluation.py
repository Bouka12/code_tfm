...
def evaluate_metric(y_true, y_pred, y_preds_proba, trtime, metric):
    """
...
def save_results_for_metric(data_dict, preds_proba_dict, preds_dict, tr_time_dict, metric, experiment):
    """
    ...
    for dataset_name, data in data_dict.items():
        y_true, y_pred, y_preds_proba, trtime = data['y_test'], preds_dict[dataset_name], preds_proba_dict[dataset_name], tr_time_dict[dataset_name]

        metric_value = evaluate_metric(y_true, y_pred, y_preds_proba, trtime, metric)
...
def evaluate_datasets(data_dict, preds_proba_dict, preds_dict, tr_time_dict, metrics, experiment):
    """
    ...
    for metric in metrics:
        save_results_for_metric(data_dict,preds_proba_dict, preds_dict, tr_time_dict, metric, experiment)
