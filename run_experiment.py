...
random_state =2025
data_dict = load_datasets(list(univariate_equal_length)[0:3])
...
def run_exp(data_dict, experiment):
    ...
    preds_dict, preds_proba_dict = train_and_predict(data_dict , extract_interval_features=interval_features_func, select_intervals= interval_selection_func,ts_representations=tsrep_funcs, random_state=random_state)
    #def train_and_predict(data_dict , extract_interval_features , select_intervals ,ts_representations, random_state):
    evaluate_datasets(data_dict=data_dict, preds_proba_dict=preds_proba_dict, preds_dict=preds_dict, metrics=metrics,experiment=experiment)
...
# test with experiment = "IS0_TSR2_IF5" 
test_exp = "IS0_TSR2_IF5"
